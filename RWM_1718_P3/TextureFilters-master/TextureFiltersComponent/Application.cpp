...
bool Application::Initialize(const char* title, int xpos, int ypos, int width, int height, int flags)
{
	...
	if (TTF_Init() == 0) 
	{
		std::cout << "SDL TFF init success" << std::endl;
	}
	else
	{
		return false;
...
{
	m_font = TTF_OpenFont("assets/kongtext.ttf", FONT_SIZE);
...
void Application::SetText(float value, int index)
{
	...
	if (index == Blend)
	{
		...
		message.erase(message.begin() + 3, message.end());
	}
	else
	{
		message = std::to_string(static_cast<int>(value));
	}
	SDL_Surface *surf = TTF_RenderText_Blended(m_font, message.c_str(), color);
	m_texture[index] = SDL_CreateTextureFromSurface(m_renderer, surf);
	int iW, iH;
	SDL_QueryTexture(m_texture[index], NULL, NULL, &iW, &iH);
	m_textureRect[index] = SDL_Rect{ m_position[index].x,  m_position[index].y, iW,iH };
